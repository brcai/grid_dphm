...
class dp:
	...
	def plotDG(self, wordParam, dx, dy):
		...
		dpcolorMap2 = [wordParam[i][0] for i in range(len(dx))]
		#for cent in centers:
		#	dpcolorMap1[cent] = 'r'
		plt.scatter(X, Y, c=dpcolorMap1, s=100)
		...
		plt.figure(2)
		plt.scatter(dx, dy, c=dpcolorMap2, s=20)
		...
		for word in aWords:
			...
			for cand in bWords:
				...
				if cand in wordNeighbour:
					if ((dist*((pa/pword)*lasso))/dc + (dist*((pb/pcand)*lasso))/dc)/2 * np.exp((max(paAverage, pbAverage)-min(paAverage, pbAverage))/max(paAverage, pbAverage))<= 1: 
					#if (dist*((pa/pword)*lasso))/dc + (dist*((pb/pcand)*lasso))/dc <= 2:
	...
	def plotClusterRes(self, wordCluster, dataVecs, centres, finalRes, dir, num):
		colors = cm.rainbow(np.linspace(0, 1, 5 + 1))
		#plt.scatter()
		...
		#plt.figure(1)
		#plt.scatter(dx, dy, marker='.', s=150, c='k')
		cx = []
		cy = []
		...
		plt.yticks([])
		plt.axis('off')
		plt.text(16.5, 4.5, r'$NMI=$'+"0.8277", fontsize=15, color='black')
		plt.savefig("C:\\study\\8data\\"+dir.split('.')[0]+"_dpha.png", bbox_inches='tight', pad_inches = 0)
		#plt.savefig("C:\\study\\dgf\\"+"dgf_org_1.png")
		plt.show()
		return

	def plotCenter(self, rawCluster, wordCluster, dx, dy, id, num, centers, boarders, dc, kl):
		colors = cm.rainbow(np.linspace(0, 1, 20))
		S = [100 for i in range(len(dx))]
		m = ['.' for i in range(len(dx))]
		plt.figure(1)
		dpcolorMap1 = ['g' for i in range(len(dx))]
		cx = []
		cy = []
		dpcolorMap2 = []
		tmpColor = 'r'
		...
		for itm in centers:
			...
			cx.append(dx[itm])
			cy.append(dy[itm])
			dpcolorMap2.append(tmpColor)
		plt.scatter(dx, dy, c=dpcolorMap1, s=S, marker='.', alpha=0.3, edgecolor='g', zorder=1)
		plt.scatter(cx, cy, c=dpcolorMap2, s=350, marker='+', zorder=2, linewidth='2.5')
		plt.xticks([])
		plt.yticks([])
		plt.axis('off')
		#plt.xlabel('X',fontsize=15)
		...
		plt.text(0, 5.5, r'$dc=$'+str(dc), fontsize=25, color='black')
		plt.savefig("C:\\study\\linkp\\"+str(dc)+kl+".png",bbox_inches='tight', pad_inches = 0)
		plt.show()
		return
	...
	def run(self, dir, dc, lasso, kl):
		...
		#self.plotClusterRes(wordCluster, dataVecs, centers, finalRes, dir, num, id)
...
if __name__ == "__main__":
	...
	#inst.run('dgf_2.txt', 0.1, 0.65, 'nl')
	#inst.run('flame.txt', 0.1, 0.65, 'nl')
	#inst.run('horn.txt', 0.05, 0.15, 'nl')
	#inst.run('tmp.txt', 0.03, 1000000, 'nl')
	#inst.run('tmp.txt', 0.03, 0.5, 'nl')
