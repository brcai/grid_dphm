...
if __name__ == "__main__":
	dx, dy, id, num = read('spiral.txt')
	dataVecs = [[dx[i], dy[i]] for i in range(len(dx))]
	db = MeanShift(bandwidth=3.2).fit(np.array(dataVecs))
	dlabel = db.labels_
	tmp = 10
	dd = []
	for itm in dlabel:
	...
	print(arsTmp)
	'''
	datasets = [['spiral.txt', 3, 'nl'], ['Compound.txt', 5, 'nl'],['pathbased.txt', 3, 'nl'],['jain.txt', 2, 'nl'],['Aggregation.txt', 7, 'nl'],['flame.txt', 2, 'nl'],
			 ['R15.txt', 15, 'gs'],['D31.txt', 31, 'gs']]
	for dataset in datasets:
		...
		for i in frange(0.1, 1, 0.01):
			for j in range(50, 500, 50):
				bandwidth = estimate_bandwidth(np.array(dataVecs), quantile=i, n_samples=j)
				db = MeanShift(bandwidth=bandwidth, bin_seeding=True).fit(np.array(dataVecs))
				dlabel = db.labels_
				tmp = 10
				dd = []
				for itm in dlabel:
