...
if __name__ == "__main__":
	...
	plt.savefig("C:\\study\\image\\84.png",bbox_inches='tight', pad_inches = 0)
	plt.figure(2)
	pre = [[np.abs(i[j]-255) for j in range(len(i))] for i in weak]
	plt.imshow(pre, cm.gray)
	plt.savefig("C:\\study\\image\\84_pre.png",bbox_inches='tight', pad_inches = 0)
	#plt.show()
	plt.figure(3)
	dataTmp = []
	for i in range(len(weak)):
		for j in range(len(weak[0])):
			if weak[i][j]>130: dataTmp.append([i,j,weak[i][j]])
	...
	#inst = dpOrig()
	#dlabel, valid, centers = inst.eval(0.1, 4, 'nl', dataTmp)
	...
	#plt.savefig("C:\\study\\image\\84_dpha.png",bbox_inches='tight', pad_inches = 0)
	plt.savefig("C:\\study\\image\\84_dp.png",bbox_inches='tight', pad_inches = 0)
	plt.show()
