...
if __name__ == "__main__":
	...
	datasets = [['pathbased.txt', 3, 'nl'],['D31.txt', 31, 'gs'], ['Compound.txt', 5, 'nl'],['R15.txt', 15, 'gs'],['jain.txt', 2, 'nl'],['Aggregation.txt', 7, 'nl'],['flame.txt', 2, 'nl'],
			 ['spiral.txt', 3, 'nl']]
	for dataset in datasets:
		...
		minp = 0.0
		ars = 0.0
		for i in frange(0.5, 0.95, 0.05):
			for j in range(-100, 100, 10):
				db = AffinityPropagation(damping=i, preference=j).fit(dataVecs)
				dlabel = db.labels_
				tmp = 50
				dd = []
				for itm in dlabel:
