...
if __name__ == "__main__":
	...
	plt.savefig("C:\\study\\image\\22.png",bbox_inches='tight', pad_inches = 0)
	plt.figure(2)
	pre = [[np.abs(i[j]-255) for j in range(len(i))] for i in weak]
	plt.imshow(pre, cm.gray)
	plt.savefig("C:\\study\\image\\22_pre.png",bbox_inches='tight', pad_inches = 0)
	plt.figure(3)
	dataTmp = []
	for i in range(len(weak)):
		for j in range(len(weak[0])):
			if weak[i][j]>150: dataTmp.append([i,j,weak[i][j]])
	
	tmpx = [data[0] for data in dataTmp]
	tmpy = [data[1] for data in dataTmp]

	#inst = dp()
	#dlabel, valid, centers = inst.eval(0.05, 2.5, 'nl', dataTmp)
	#inst = dpOrig()
	#dlabel, valid, centers = inst.eval(0.14, 3, 'nl', dataTmp)
	dbs = DBSCAN(eps=1.4, min_samples=2).fit(dataTmp)
	dlabel = dbs.labels_
	print('start clustering')
	#dlabel, valid, centers = inst.eval(0.2, 5, 'nl', dataTmp)
	...
	#plt.savefig("C:\\study\\image\\22_dpha.png",bbox_inches='tight', pad_inches = 0)
	plt.savefig("C:\\study\\image\\22_dp.png",bbox_inches='tight', pad_inches = 0)
