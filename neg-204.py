...
if __name__ == "__main__":
	...
	plt.savefig("C:\\study\\image\\204.png",bbox_inches='tight', pad_inches = 0)
	plt.figure(2)
	pre = [[np.abs(i[j]-255) for j in range(len(i))] for i in weak]
	plt.imshow(pre, cm.gray)
	plt.savefig("C:\\study\\image\\204_pre.png",bbox_inches='tight', pad_inches = 0)
	plt.figure(3)
	dataTmp = []
	...
	#inst = dp()
	#dlabel, valid, centers = inst.eval(0.08, 2, 'nl', dataTmp)
	inst = dpOrig()
	dlabel, valid, centers = inst.eval(0.2, 3, 'nl', dataTmp)
	...
	plt.savefig("C:\\study\\image\\204_dp.png",bbox_inches='tight', pad_inches = 0)
	plt.show()
